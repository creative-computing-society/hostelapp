...
class Hostel(db.Model):
    ...
    mess_members = db.Column(
        db.String(50), nullable=True)  # format - 'member1:member2:member3'
    student_representatives = db.Column(
        db.String(50), nullable=True)  # format - 'student1:student2:student3'
...
class Utility():
    ...
    @staticmethod
    def create_hostels(name, warden, caretaker, mess_members,
    ...
    if request.method == 'POST':
        ...
        if Utility.validate_email(credentials['email']) is False:
            return jsonify(status='401',
                           error='Invalid Email or Emailalready in use')

        if Utility.validate_phone_number(int(
                credentials['phone_number'])) is False:
            return jsonify(
                status='401',
                error='Invalid Phone Number or Phone Number already in use')

        if Utility.validate_roll_number(int(
                credentials['roll_number'])) is False:
            return jsonify(
                status='401',
                error='Invalid Roll Number or Roll Number already in use')
        ...
        try:
            user = User(username=credentials['name'],
                        email=credentials['email'],
                        password=credentials['password'],
                        phone_number=credentials['phone_number'],
                        roll_number=credentials['roll_number'],
                        hostel=credentials['hostel'])
            db.session.add(user)
            db.session.commit()
...
def login():
    ...
    if request.method == 'POST':
        ...
        user = User.query.filter_by(
            roll_number=credentials['roll_number']).first()
        if not user:
...
def register_complaint():
    ...
    if request.method == 'POST':
        ...
        try:
            complaint = Complaint(category=complaint['category'],
...
def fetch_complaints():
    ...
    if request.method == 'POST':
        ...
        complaints = Complaint.query.filter_by(
            hostel_name=json['hostel']).all()


@app.route('/v1.0/fetch_menu',
           methods=['GET', 'POST'])  # Work with caution, mindfux ahead
def fetch_mess_menu():
    '''
    ...
    if request.method == 'POST':
        ...
        json = request.get_json()
        menu = Mess_Menu.query.filter_by(hostel_name=json['hostel'],
                                         week=int(json['week'])).first()
        if menu:
            ...
            return jsonify({
                'food':
                dict1['food'][json['category']][json['day']],
                'timings':
                dict1['timings'][json['category']][json['day']]
            })
        return jsonify(error='menu not found')


@app.route('/v1.0/update_menu', methods=['GET', 'POST']
           )  # if menu is not created yet, create else updating takes place.
def update_menu():

    if request.method == 'POST':
        menu_details = request.get_json()

        menu = Mess_Menu.query.filter_by(hostel_name=menu_details['hostel'],
                                         week=int(
                                             menu_details['week'])).first()
        print(menu)
...
Utility.create_hostels(
    'H', 'ABC', 'ABC', 'Mankaran Singh::Udhbav Oberoi',
    'Mankaran Singh::Udhbav Oberoi::Nikhil'
)  # name, warden, caretaker, mess_members,student_representatives
Utility.create_hostels('B', 'ABC', 'ABC', 'Mankaran Singh::Udhbav Oberoi',
                       'Mankaran Singh::Udhbav Oberoi::Nikhil')
